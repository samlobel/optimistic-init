...
import torch
import numpy as np
from rbfdqn.tasks.sibrivmaze.mazes import mazes_dict, make_crazy_maze, make_experiment_maze, make_hallway_maze, make_u_maze

import gym


class Env(gym.Env):
    def __init__(self, n=None, maze_type='square_large', use_antigoal=True, ddiff=False, ignore_reset_start=False, fixed_goal=None):
        self.n = n
        print(f"Using fixed goal {fixed_goal} for PointMaze")
    ...
    def dist(goal, outcome):
        ...
        # return np.sqrt(np.sum(np.exp2(goal - outcome)))
    ...
    def action_space(self):
        assert isinstance(self.action_size, int)
        return gym.spaces.Box(-self.action_range, self.action_range, shape=(self.action_size,))
        # action_range = self.action_range

    @property
    def observation_space(self):
        """
        I could be more exact about this for the maze I care about, but I don't THINK it matters.
        """
        assert isinstance(self.state_size, int)
        return gym.spaces.Box(low=0., high=10., shape=(2,), dtype=np.float32)
        # return gym.spaces.Box(-np.inf, np.inf, shape=(self.state_size,))      

    @property
    def state(self):
        return np.array(self._state['state'].reshape(-1))
        # return self._state['state'].view(-1).detach().numpy()

    @property
    def goal(self):
        return np.array(self._state['goal'].reshape(-1))
        # return self._state['goal'].view(-1).detach().numpy()

    @property
    def antigoal(self):
        return np.array(self._state['antigoal'].reshape(-1))
        # return self._state['antigoal'].view(-1).detach().numpy()
    ...
    def next_phase_reset(self):
        return {'state': self._state['s0'].detach().numpy(), 'goal': self.goal, 'antigoal': self.achieved}

    @property
    def sibling_reset(self):
        return {'state': self._state['s0'].detach().numpy(), 'goal': self.goal}

    def reset(self, state=None, goal=None, antigoal=None):
        if state is None or self._ignore_reset_start:
            s_xy = self.to_numpy(self.maze.sample_start())
        else:
            s_xy = self.to_numpy(state)
        if self.fixed_goal is not None:
            g_xy = self.to_numpy(self.fixed_goal)
        elif goal is None:
            if 'square' in self.maze_type:
                g_xy = self.to_numpy(self.maze.sample_goal(min_wall_dist=0.025 + self.dist_threshold))
            else:
                g_xy = self.to_numpy(self.maze.sample_goal())
        else:
            g_xy = self.to_numpy(goal)

        if antigoal is None:
            ag_xy = self.to_numpy(g_xy)
        else:
            ag_xy = self.to_numpy(antigoal)
    ...
    def step(self, action):
        try:
            ...
            raise
        self._state['prev_state'] = self.to_numpy(self._state['state'])
        self._state['state'] = self.to_numpy(next_state)
        self._state['n'] += 1
