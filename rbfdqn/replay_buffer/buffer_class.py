...
class buffer_class:
    def __init__(self, max_length, env):
        ...
        self._storage = ReplayBuffer(max_length, env_dict)

    def append(self, s, a, r, done, sp):
        self._storage.add(**self.before_add(obs=s, act=a, rew=r, done=done, next_obs=sp))

    def sample(self, batch_size):
        batch = self._storage.sample(batch_size)
        s_matrix = batch['obs']
        a_matrix = batch['act']
        r_matrix = batch['rew']
    ...
    def __len__(self):
        return self._storage.get_stored_size()